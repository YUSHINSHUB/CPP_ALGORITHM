...
This C++ code simulates a game similar to "Snakes and Ladders". Given a board with some ladders and snakes, the code calculates the minimum number of dice rolls required to reach the 100th cell starting from the 1st cell. If a player lands on the start of a ladder or snake, they immediately move to its end.

이 C++ 코드는 "뱀과 사다리"와 유사한 게임을 시뮬레이션합니다. 몇몇의 사다리와 뱀이 있는 보드가 주어진 경우, 코드는 1번 칸에서 시작하여 100번 칸에 도달하기 위해 필요한 주사위 굴림의 최소 횟수를 계산합니다. 플레이어가 사다리나 뱀의 시작 부분에 도착하면 즉시 그 끝으로 이동합니다.
*/
